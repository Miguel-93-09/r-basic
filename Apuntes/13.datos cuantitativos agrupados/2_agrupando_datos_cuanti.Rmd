...
-   regla de Scott : primero calcula amplitud teorica de clases, donde $\bar{s}$ es la desv tipica muestral $$A_S = 3.5  \cdot \bar{s} \cdot n^{-1/3}$$ entonces: $$k = \left \lceil {\frac {\max(x) - \min(x)}  {A_S}}  \right \rceil $$ 

-   Regla de Freedman-diaconis: analogo a anterior primero amplitud teorica, luego clases $$A_{FD} = 2  \cdot (Q_{.75} - Q_{.25}) \cdot n^{-1/3} $$ entonces: $$ k = \left \lceil {\frac {\max(x) - \min(x)}  {A_{FD}}}  \right \rceil$$
...
los intervalos seran cerrados por izquierda y abiertos por derecha por convencion
$$(L_k,L_{k+1}]$$
los mismos se calculan como

$$ L_1 = \min(x) - \frac{1}{2} \cdot \text{medida de precision }$$
$$L_k = L_1 +(k-1)A , \quad i=2,\ldots,k+1$$
de forma rque garantizmaos que ningun extremo coincida con algun valor del vector por la precision mayor.
...
$$X_i = X_1+ (i-1)A , \quad i=2,\ldots,k$$
donde
$$X_1 = \frac{L_1+L_2}{2} $$
